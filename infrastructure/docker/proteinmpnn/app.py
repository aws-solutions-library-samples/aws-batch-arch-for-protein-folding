...
from protein_mpnn_utils import parse_fasta
import argparse
import logging
import os
import json
...
def get_seq_path(args):
    target_id = os.path.splitext(os.path.basename(args.pdb_path))[0]
    return(os.path.join(args.out_folder, "seqs", target_id + ".fa", ))

def parse_seqs(file):
    headers, seqs = parse_fasta(file)
    summary, headers = headers[0], headers[1:]
    target, seqs = seqs[0], seqs[1:]

    designs = []
    for h, s in zip(headers, seqs):
        header_list=h.split(", ")
        d = {k:v for k, v in [tuple(i.split('=')) for i in header_list]}
        d.update({'seq': s})
        designs.extend(d)
...
def lambda_handler(event, context):
    ...
    for key, value in event.items():
        ...
        option_list.append(str(value))

    args = argparser.parse_args(option_list)
    main(args)

    seq_path = get_seq_path(args)
    output = parse_seqs(seq_path)
...
if __name__ == "__main__":
    event = {
        "pdb_path": "/var/task/tests/3HTN.pdb",
        "pdb_path_chains": "A B",
    ...
    lambda_handler(event=event, context="test")