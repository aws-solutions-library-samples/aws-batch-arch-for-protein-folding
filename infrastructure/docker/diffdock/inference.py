...
parser.add_argument('--so_distribution_cache_dir', type=str, default=None, help='Path to the SO(2) and SO(3) distribution cache files')
parser.add_argument('--esm_param_model_dir', type=str, default=None, help='Path to the ESM-2 model params used to generate protein embeddings.')

args = parser.parse_args()

if args.protein_sequence is not None:
    raise AttributeError('Protein sequence inputs are not supported at this time. Please generate a protein structure file with ESMFold (or other algorithm) and submit it via --protein_path')

if args.so_distribution_cache_dir is not None:    
    for f in glob(f"{args.so_distribution_cache_dir}/.*.npy"):
        dest = os.getcwd()
        print(f"Copying {f} to {dest}")
        shutil.copy2(f, dest)
if args.esm_param_model_dir is not None:    
    for f in glob(f"{args.esm_param_model_dir}/*.pt"):
