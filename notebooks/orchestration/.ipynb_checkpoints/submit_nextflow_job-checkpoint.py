...
nextflow_script="run_rfdesign_esmfold_multiple__sequences.nf"
...
os.system(f'aws s3 cp  pd1_demo/pd1.pdb s3://{bucket}/pd1-demo/') #copy pdb structures
os.system(f'aws s3 cp  pd1_demo/pdl1.pdb s3://{bucket}/pd1-demo/') #copy pdb structures
os.system(f'aws s3 cp --recursive bin/ {my_asset_uri}/bin/') #copy bin to s3 in the bin directory
...
response = client.submit_job(
    ...
    #jobQueue='GravitonOnDemandJobQueue-OeZC9xBMTuPVZ6VG',
    jobQueue=orchestrator_compute_environment, #modify this to your own JobQueue
    jobDefinition=nextflow_job_definition, #modify this to your own JobQueue
