...
def test_rfdiffusion_job_init():
    ...
    job_name = "SuperfoldJob" + datetime.now().strftime("%Y%m%d%s")
    params = {
    ...
    new_job = RFDiffusionJob(
        ...
        job_name = job_name,
        memory=15,
        input_s3_uri = f"s3://{bucket}/superfold/superfold.fa",
        output_s3_uri = f"s3://{bucket}/superfold/outputs/",
        params=params
    )
    assert new_job.job_definition_name == "SuperfoldJobDefinition"
    assert new_job.input_s3_uri == f"s3://{bucket}/superfold/superfold.fa" # Todo, Path?
    assert new_job.output_s3_uri == f"s3://{bucket}/superfold/outputs/"
...
def test_rfdiffusion_job_submission(batch_environment):
    ...
    job_name = "SuperfoldDiffusionJob" + datetime.now().strftime("%Y%m%d%s")
    params = {
        "inference.num_designs": "3",
        "contigmap.contigs": "[10-40/A163-181/10-40]"
    }
